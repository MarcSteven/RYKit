...
public extension UITableView {
    ...
    /// remove eliminate extra separators
...
public extension UITableView {
    /// Select rows
    func selectRows(in indexSet: NSIndexSet?, inSection section: Int, animated: Bool, scrollPosition: UITableView.ScrollPosition) {
           indexSet?.enumerate({ idx, stop in
               self.selectRow(at: IndexPath(row: idx, section: section), animated: animated, scrollPosition: scrollPosition)
           })
       }
}
...
public extension UIViewController {
    ...
    func isPresenting() ->Bool {
        return self.navigationController?.presentingViewController == self
    }
}

public extension Timer {
    func invalidateTimer() ->Any? {
        if isValid {
            invalidate()
        }
        return nil
...
public extension UIView {
    func increaseFrameBy(x: CGFloat, y: CGFloat, width: CGFloat, height: CGFloat) {
        let currentRect = frame
        frame = CGIncreaseRect(currentRect, x, y, width, height)
    }
}

func CGIncreaseRect(_ rect: CGRect, _ x: CGFloat, _ y: CGFloat, _ width: CGFloat, _ height: CGFloat) -> CGRect {
    return CGRect(x: rect.origin.x + x, y: rect.origin.y + y, width: rect.size.width + width, height: rect.size.height + height)
}

func CGIncreasePoint(_ point: CGPoint, _ x: CGFloat, _ y: CGFloat) -> CGPoint {
    return CGPoint(x: point.x + x, y: point.y + y)
}

func CGIncreaseSize(_ size: CGSize, _ width: CGFloat, _ height: CGFloat) -> CGSize {
    return CGSize(width: size.width + width, height: size.height + height)
}

public extension UIApplication {
    var shouldBeOnLeft:Bool {
...
public extension UIView {
    ...
    func roundCorner(_ corners:UIRectCorner,
                      toRadius radius:Float) {
    ...
    func zeroLayerOpacity() {
        layer.opacity = 0
    }
    func fadeInAndUpWithDuration( duration:Double,
                                  delay:Double) {
        layer.transform = CATransform3DMakeTranslation(0, 18, 0)
        UIView.animate(withDuration: duration, animations: <#T##() -> Void#>, completion: <#T##((Bool) -> Void)?##((Bool) -> Void)?##(Bool) -> Void#>)
    }
}
