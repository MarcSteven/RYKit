...
public class TabBarController :UIViewController {
    ...
    /// insert to viewController
    /// - Parameters:
    ///   - parent: viewController
    ///   - names: names of icon
    ///   - sNames: selected icon name
    /// - Returns: return UITabBarController
    ...
    /// buttons container height constraint initial constant
    internal var buttonsContainerHeightConstraintInitialConstant:CGFloat!
    
    /// selection indicator height constraint
    ...
    @objc func tabButtonAction(button:UIButton){
        if let index = self.buttons.firstIndex(of: button){
            ...
            if let _ = delegate?.tabBar(self, systemSoundIdForButtonAtIndex: index), !isAnimating{
